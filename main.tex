...
\todoi{Дописать введение!}

{
\Russian
Эта статья является продолжением серии итоговых статей по эксперименту с установкой СФЕРА-2, начатой статьей~\cite{SPHERE21}, в которой описывались условия проведения эксперимента и контроль параметров установки.
}

%In this paper we describe the data 
{\Russian
В настоящей статье мы описываем процедуру сбора данных, включая вопрос выставления пороговых уровней, предобработку сырых данных и отбор событий для анализа в качестве событий ШАЛ.
}
...
\centering
    ...
    \includegraphics[width=0.48\textwidth]{figs/fig1_new.pdf}%
    \vspace{-0.5pc}
...
The \mbox{SPHERE-2} detector is a compact optical device elevated with the balloon to altitudes up to 1~km above the Earth surface to measure the EAS Cherenkov light reflected from the snow covered surface. 
...
The PMT mosaic is located near the mirror focal surface and records the Cherenkov light reflected from the snow surface below the detector. PMTs are arranged in the hexagonal structure on the spherical surface as shown in Fig.~\ref{fig:mosaic_sine}. 
...
{\Russian
На рисунке~\ref{fig:int_discr} показана схема интегрирующего дискриминатора измерительного канала для формирования запроса триггера. Данная схема реализована внутри микросхем FPGA (Field-Programmable Gate Array) на всех измерительных каналах. В составе каждого канала имеются по два FADC включенных в режиме чередования. Для обеспечения стабильности работы интегрирующего дискриминатора используется только один из этих двух FADC. Данные с этого FADC поступают на регистр RG1. Данные записываются в регистр по переднему фронту сигнала Clock FADC. Аналогичным образом сигнал Clock фиксирует входные (слева) данные на выходах (справа) всех регистров RG0-RG4. Так как регистры RG1--RG4 включены последовательно, то на входах сумматора (обозначены на схеме тремя трапециями) всегда представлены данные о четырех последних измерениях FADC. Длительность измерительно такта Clock составляет 25нс, что достаточно для получения результата суммирования на входе RG0. Таким образом требуется 5 тактов Clock (соответствует 125нс) для формирования истинного результата на выходе RG0. После суммирования четырех 10-ти разрядных слов результат представляется в виде одного 12-ти разрядного слова. Результат сравнивается с программно заданным значением порога дискриминации с помощью цифрового компаратора. В случае превышения порога компаратор выдаёт сигнал на плату триггера на запрос триггера. Если сигналы запроса триггера от каналов выполняют условия срабатывания триггера, то выполняется процедура сохранения данных в буфер.    
}

\subsection{Threshold autoset procedure}

%% блок-схема выставления порогов (Бонвеч)
%%%% === set_levels_diagramm ===
\begin{figure}[t]
    \centering
    \includegraphics[width=0.30\textwidth]{figs/set_levels_algorithm.pdf}
    \vspace{-1pc}%
    \caption{The scheme of the procedure to set the discriminator threshold levels.}
\label{fig:set_levels_algorithm}
%\todoi{Схема выставления порогов и ее описание}
\end{figure}

Значения порога дискриминации во всех измерительных каналах задаются программой, исходя из конкретных условий работы установки, таких, как выставленное напряжение питания, средний ток, и т.д. Основная цель процедуры - выставление минимально возможных порогов дискриминатора для достижения минимального энергетического порога при данных условиях проведения эксперимента. Однако, чем меньше пороговый уровень, тем выше темп счета канала, а следовательно и всей установки. Установка может регистрировать события с предельно возможной частотой 6~Hz. Это означает, что при условии L3 темп счета каждого отдельного измерительного канала не должен превышать значения 1~Hz. 

The procedure of the discriminator threshold levels determination is described below and is shown in the Fig.~\ref{fig:set_levels_algorithm}. The diagrams in Fig.~\ref{fig:set_levels_algorithm} and Fig.~\ref{fig:get_event_algorithm} are created with the visual algorithmic language DRAKON~\cite{drakon} using \href{https://drakonhub.com}{drakonhub} tool.

Выставление порогов начинается с выставления максимально возможного порога в 4096 единиц кода во всех измерительных каналах. Это значение равно максимальной амплитуде сигнала, которая может прийти на компаратор, как сумма регистров G1–RG4 in the Fig.~\ref{fig:int_discr}.

После выставления уровней установка запускается на счёт в течение нескольких (4) секунд. Рассматривается количество срабатываний каждого канала с целью установить темп счета, близкий к целевому. Поскольку достичь целевого темпа счета в точности может оказаться невозможным, определяются два значения: минимальный и максимальный допустимые темпы, они отличаются от целевого на множители 0.9 и 1.1 соответственно. Eсли темп счета канала больше, чем максимально допустимый, то значение порогового уровня увеличивается на величину $dL$ единиц кода, чтобы снизить темп счета. Если темп счета меньше минимально допустимого, значение порога уменьшается на $dL$. После этого процедура повторяется. Значение $dL$ на первом шаге процедуры равно 2048, на каждом последующем шаге уменьшается в два раза, пока не достигнет значения $dL = 1$. В этот момент пороги считаются выставленными в первом приближении. Вся процедура проходит за 12 шагов, и занимает меньше минуты. 

После этого еще 10 раз проводится подстройка порогов с $\mathrm{d}L = 1$. Пороги могут быть как увеличены на 1 единицу кода, так и уменьшены.

В некоторых измерительных каналах к этому моменту складывается такая ситуация, что уменьшение порога на 1 единицу приводит к уменьшению темпа счета канала ниже минимально допустимого, а увеличение на 1 единицу кода - к увеличению темпа счета выше предельно допустимого. Если оставить этот канал с маленьким порогом, он приведет к "забою" всей установки шумовыми срабатываниями. Поэтому дополнительно проводится еще 10 раз процедура коррекции порога, при которой порог может изменяться только в сторону увеличения.


По окончании полной процедуры выставления порогов в каждом измерительном канала установлен пороговый уровень дискриминатора, который обеспечивает темп счета канала не более, чем максимально допустимый. Вся процедура длится менее 3 минут. Значение параметра целевого темпа счета задается программе автоматического управления работой установки через конфигурационный файл параметров. При необходимости, процедура автоматического выставления порогов дискриминатора может быть запущена оператором в любой момент во время работы установки. 
...
% блок-схема триггера
The trigger system (TS) of the SPHERE-2 detector produces a `record' signal when the trigger condition is satisfied. The trigger condition was: the signal exceeding of the threshold level
in any $L$ neighbours PMTs
during 1~${\mu}$s time window  --- so called `local' trigger $LN$ or in any $M$ PMTs over the mosaic --- `global' trigger $GM$. The number of channels to exceed the threshold level was varied from flight to flight to decrease the energy threshold, but the most of the data was obtained under trigger condition `$L3$ $or$ $G5$' then at least one of these two trigger conditions have to be satisfied. 

The posterior analysis shows that some events was recorded under trigger $L2$ although the condition $L3$ was set.
...
% определение frame - 112 х 2 х 510 + telemetry
When the TS produced a `record' signal the DAQ started recording 12.8~$\mu$s of signal from the delay line including capturing $\sim6~\mu$s of the signal prior to the trigger signal and formed an `event' data in the buffer. One detector `event' data frame contains a 12.8~$\mu$s record of the PMT DAQ signal data from measuring channels with 12.5~ns discretization, in total 1020 10-bit signal values per every of 109~PMT channels. If the signal value exceeded the 10-bit number the 11$^{th}$ bit of the recorded signal value number was set to 1 to indicate the overflowing of the FADC counter and to 0 otherwise (by default). The highest 12$^{th}$ bit of signal value number is set to 1 if the PMT signal exceeds the trigger level. 

The buffer capacity is enough to keep four `event' frames.

The main program permanently checks the buffer status and if it is not empty saves data from buffer to a binary data file. The scheme of the buffer data reading procedure is shown on the figure~\ref{fig:get_event_algorithm}. If there are four events in the buffer the trigger is prohibited and buffer is blocked to write to prevent buffer overflow during reading procedure. If buffer is overflowed then it will be cleared after reading four events. 
Every event data is read from buffer and is written to the binary data file. Together with the buffer data the additional information is written to a binary file: the event number, local computer time, two detector inclination angles, GPS coordinates, altitude, UTC time, trigger time, magnetometer angle and the average anode currents for all 109~PMT. 

One `event' information takes 220~kBytes. All the data is saved to a binary file on the detector on-board computer SSD drive. From the primary binary files new text dataframe files were created during offline data preprocessing. Preprocessing includes baseline estimation and substraction, channel time drift detection and correction, noise subtraction and calibration described in the next section. %see~\ref{sect:preprocessing}.
...
\todoi{Добавить шум для 2012 и 2011 лет.}
\end{figure}
...
Сумму всех событий с учётом синхронизации шумовых импульсов см. на noise full length (или выделенный повторяющийся сегмент на noise segment). Шаг повторений 252 бина, что соответствует частоте 317460-317800 Гц (но это уже не ко мне, что там так шумит). В остатке имеем, что во всех кадрах присутствует вот такая наводка in the Fig.~\ref{fig:pulse_noise_segment} c шагом повторений 252 бина по времени. 
...
The SPHERE-2 detector recorded more than 30 thousands trigger event frames but not all of them was caused by signals of the EAS Cherenkov light. The separation of EAS events is the task of this section. 

Examples of data frames detected by the SPHERE-2 detector are presented in the Figs.~\ref{fig:frames}, \ref{fig:pulses} and \ref{fig:mosaic_sum}. They are of different dataframe types. 

The Fig.~\ref{fig:frames} presents events as a full map of signal intensities in all PMT channels during the frame time. The x-axis on the Fig.~\ref{fig:frames} is the PMT number $N$ according to Fig.~\ref{fig:mosaic_sine} and the y-axis is the 12.5~ns time bin number $T$. The color intensity of every point in the Fig.~\ref{fig:frames} is proportional to the logarithm of the signal of the photomultiplier $N$ in the time bin $T$. Few measuring channels sometimes failed or worked incorrectly. The data of such channels were not processed and are visible in the figures as zeros.

Fig.~\ref{fig:pulses} shows signal pulses in channels with the largest signal magnitudes in the event frame. And the last Fig.~\ref{fig:mosaic_sum} demonstrates an integral signal for every optic channel. The integration was made as a sum of signals in time limits indicated in the Fig.~\ref{fig:pulses} for every frame respectively. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Data frames

%%%%  2012-3559 !!! линия и змейка !!!! !!!!! !!!!!! !!!!!!!
...
The `event' frame 11588 on the Fig.~\ref{fig:frames}a, \ref{fig:pulses}a and \ref{fig:mosaic_sum}a is an EAS-caused event, it is an image of reflected from the snow surface EAS Cherenkov light. The event structure is typical for Cherenkov light frames as it was described in our paper dedicated to simulation of the SPHERE-2 detector experiment~\cite{Ant19}. The event 11588 was detected from the detector altitude 702~m above the show surface. The inclination of the detector was $1.5^{\circ}$.

On the Fig.~\ref{fig:mosaic_eas} this event is represented step by step as a sequience of images with 12.5~ns interval. It is clearly seen how the signal of Cherenkov light reflected from the snow goes through the PMT mosaic.

The frame 11589 on the Fig.~\ref{fig:frames}b, \ref{fig:pulses}b and \ref{fig:mosaic_sum}b is the calibration event following the event 11588. It has the special time signal structure and big magnitudes in all channels caused by LEDs flushes. Calibration frames are separated to calculate the calibration coefficients for all measuring channel as desctibed in~\cite{Ant16}.
...
A mysterious event type like 12114 is presented on Fig.~\ref{fig:frames}d, \ref{fig:pulses}d and \ref{fig:mosaic_sum}d. The event frame has long pulses in numerous channels. The duration of pulses is more than 6 ${\mu}s$ and exceed the frame time capacity.  This `event' may be triggered by crosstalks of PMTs. Or by some long-time events on the surface or in the atmosphere above the detector. The authors favorite version is that these events are caused by light flashes from the electrical discharges accompanying the Baikal lake ice cracking (triboluminescence).
...
Calibration frames have not special tags and are separated by the large signal sum and the large pulse duration. The detector was configured to make calibration frame after every trigger event frame. But sometimes during experiment the counting rate of the detector exceeded reception capacity of the detector buffer and it caused the buffer overflow and a failure in its operation so the calibration frame was not recorded. Therefore not exact half of recorded events are calibration ones. Thus from all 3813 frames recorded in 2013 run near the half of them (1901) was calibration ones. 

The failure of the buffer is seen also as a trigger bit location outside of [220, 300] time interval in the channel data. So frames with trigger time bin less than 220 and more than 300 time bin was deleted from the further consideration. There was 130 frames with this trigger failure.
