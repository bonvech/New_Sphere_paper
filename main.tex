...
{
\Russian
Эта статья является продолжением серии итоговых статей по эксперименту с установкой СФЕРА-2, начатой статьей~\cite{SPHERE21}, в которой описываются условия проведения эксперимента.
}
...
% блок-схема триггера
The trigger system (TS) of the SPHERE-2 detector produces a `record' signal when the trigger condition is satisfied. The trigger condition was: 1~${\mu}$s time window \todoi{ заполнить пропуск в логике} --- so called `local' trigger $LN$ or in any $M$ PMTs over the mosaic --- `global' trigger $GM$. The number of channels to exceed the threshold level was varied from flight to flight to decrease the energy threshold, but the most of the data was obtained under trigger condition `$L3$ $or$ $G5$' then at least one of two trigger conditions have to be satisfied. 
